...
async def test_system_stats(nc: NATS):
    ...
    if last_event_time:
        print(f"  Last Event:    {events.get('last_event_type')} at {format_timestamp(last_event_time)}")
    ...
    if last_cmd_time:
        print(f"  Last Command:  {commands.get('last_command_type')} at {format_timestamp(last_cmd_time)}")
