...
async def test_system_shutdown_validation(nc: NATS):
    ...
    if "delay_seconds must be between 0 and 300" in error:
        print(f"✅ PASSED: Correctly rejected invalid delay")
        print(f"  Error: {error}")
    else:
    ...
    if "delay_seconds must be between 0 and 300" in response.get("error", ""):
        print(f"✅ PASSED: Correctly rejected negative delay")
    else:
        print(f"❌ FAILED: Wrong error for negative delay")
        return False
...
async def test_system_shutdown_dry_run(nc: NATS):
    ...
    response = await send_command(
        ...
        "system.shutdown",
        delay_seconds=60,
        reason="Test shutdown from Phase 3 test script"
    )
