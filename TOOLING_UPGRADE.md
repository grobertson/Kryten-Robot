...
CI runs the standard gate (`ruff check`, `black --check`, `mypy`, `pytest`) via `uv run`, and
the pre-commit hooks (`.pre-commit-config.yaml`) call the same `uv run` tools — so bumping the
versions here fixes local commits and CI together. Note: this repo's mypy target is the
`kryten` package (see `AGENTS.md` / `ci.yml`), not `.`.
